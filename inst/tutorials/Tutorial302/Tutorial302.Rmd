...
```{r p1, exercise=TRUE, exercise.lines = 25}
library(paqueteMOD)
data("beer")
...
```{r p2, exercise=TRUE, exercise.lines = 25}
library(paqueteMOD)
data("beer")
...
```{r p3, exercise=TRUE, exercise.lines = 25}
library(paqueteMOD)
data("beer")
...
ACP = prcomp(beer[,2:4], center=TRUE, scale=TRUE)
biplot(ACP, cex=0.7)
#además de la relación entre las variables, visualiza que unas marcas estan  en los ejes positivos  mientras otras se alenan
```
...
```{r p6, exercise=TRUE, exercise.lines = 25}
library(paqueteMOD)
data("animals")
