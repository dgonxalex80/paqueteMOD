...
Método de optimización matemático que permite encontrar como solución los valores de los coeficientes del modelo de regresión que minimizan la suma de cuadrados de los errores.
...
Si los $\widehat{\beta}_{i}$ se distribujen normal, entonces
...
A continuación se presenta los resultados de la estimacción MCO el modelo : `preciom = areaconst + parqueaderos + banios + habitaciones`

```{r}
library(paqueteMOD)
data(vivienda)
modelo = lm(preciom = areaconst + parqueaderos + banios + habitaciones, data=vivienda)
```
